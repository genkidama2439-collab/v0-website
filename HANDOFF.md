...
## 最新状況（2026-08-13、本番反映済み）

利用者の明示同意を条件に、匿名分析から「予約者と結合できる顧客・行動分析」へ移行した。下記の2026-07-23の作業ログは、導入前の履歴としてそのまま残している。
...
1. 分析GASへ `Code.gs` と `appsscript.json` を反映し、既存の分析シートに対して `setupAnalyticsWorkbook()` を実行、新バージョンでデプロイ済み（ユーザー確認）。
2. 予約管理GASへ `Code.gs` を反映し、新バージョンでデプロイ済み（ユーザー確認）。次回予約受信時に、既存行を消さず末尾列が自動追加される。
3. サイト変更はPR #26で `main` へ反映し、Vercel本番へデプロイ。
4. 実予約を発生させない範囲の表示・同意動作をデプロイ後に確認。通常予約、LINEログイン往復、4言語の本番相当テスト予約は別途運用確認する。
