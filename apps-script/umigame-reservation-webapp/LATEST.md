...
2. 予約受付GASで`setupReferralProgram()`を1回実行
3. Webサイトをデプロイし、署名済み紹介CookieをGASへ連携
4. 管理Webアプリの`Code.gs`・`Index.html`・`App.html`・`Styles.html`を貼り替えて新バージョンをデプロイ
5. テスト予約で紹介成果、単品⇔セット変更、カレンダー、`予約変更履歴`、LINE未送信を確認
