...
- 同意済み顧客の `Visitor ID` / `Visit ID` / `予約ファネルID` と予約番号を使い、予約前の閲覧履歴と予約台帳を結合できる実装を追加し、本番へデプロイ済み。
- 予約台帳末尾に顧客連絡先、同意記録、流入情報、端末情報、参加者の年齢・身長・体重・足サイズ、要望・アレルギー等の列を追加。
- 日本語を含む4言語のメールアドレス入力を必須化。
- Cookie・行動履歴連携の同意通知と4言語のプライバシーポリシーを追加・更新。
- フェーズ1のファネルイベント、個人情報非混入、GAS列対応の自動テストは実装済み。`visibilitychange` の誤離脱記録も廃止。
- 残る大きな優先事項は P0-1（予約送信の冪等化）、P0-2（LINE必須の運用判断）、P0-3（ドラフト保持方式）、P0-4（永続エラー表示）。

以下は監査時点の原文を残しており、「現在」の記述には上記実装前の内容が含まれる。
...
- **対応済み・本番デプロイ済み（2026-08-13）**: 日本語フォームにも必須のメールアドレス入力欄を追加した
